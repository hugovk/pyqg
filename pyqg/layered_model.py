...
class LayeredModel(model.Model):
    ...
    def _initialize_stretching_matrix(self):
        ...
        if (self.nz == 2) and (self.rd) and (self.delta):
            ...
            self.del1 = self.delta / (self.delta + 1.0)
            self.del2 = (self.delta + 1.0) ** -1
            self.Us = self.Ubg[0] - self.Ubg[1]

            self.F1 = self.rd ** -2 / (1.0 + self.delta)
    ...
    def _initialize_background(self):
        ...
        if not (self.nz == 2):
            ...
            self.f2gpi = np.array(self.rd ** -2 * (self.Hi[0] * self.Hi[1]) / self.H)[
    ...
    def _initialize_inversion_matrix(self):
        ...
        self.a = np.ma.masked_invalid(a).filled(0.0)
    ...
    def _calc_ke(self):
        ...
        for j in range(self.nz):
            ke += 0.5 * self.Hi[j] * self.spec_var(self.wv * self.ph[j])
    ...
    def _calc_eddy_time(self):
        ...
        ens = 0.0
        for j in range(self.nz):
            ens = 0.5 * self.Hi[j] * self.spec_var(self.wv2 * self.ph[j])

        return 2.0 * pi * np.sqrt(self.H / ens.sum()) / 86400
    ...
    def _initialize_model_diagnostics(self):
        ...
        self.add_diagnostic(
            ...
            function=(
                lambda self: np.abs(
                    (self.Hi[:, np.newaxis, np.newaxis] * self.qh).sum(axis=0)
                )
                ** 2
                / self.H
            ),
        ...
        self.add_diagnostic(
            ...
            function=(lambda self: self.wv2 * (np.abs(self.phn) ** 2) / self.M ** 2),
            units="",
            dims=("lev", "l", "k"),
        )
        ...
        self.add_diagnostic(
            ...
            function=(
                lambda self: self.kdi2[1:, np.newaxis, np.newaxis]
                * (np.abs(self.phn[1:, :, :]) ** 2)
                / self.M ** 2
            ),
            units="",
        ...
        self.add_diagnostic(
            ...
            function=(
                lambda self: (self.f2gpi * np.abs(self.ph[:-1] - self.ph[1:]) ** 2).sum(
                    axis=0
                )
                / self.H
            ),
        ...
        self.add_diagnostic(
            ...
            function=(
                lambda self: -(
                    self.Hi[:, np.newaxis, np.newaxis]
                    * ((self.ikQy - self.ilQx) * (self.Sph.conj() * self.ph)).real
                ).sum(axis=0)
                / self.H
            ),
            units="",
