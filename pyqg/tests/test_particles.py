...
class ParticleTester(unittest.TestCase):
    ...
    def test_integration(self, rtol=1e-14, atol=1e-10):
        ...
        for om in [0.1, 1.0, 10.0]:
            ...
            np.testing.assert_allclose(
                np.array([lpa.x, lpa.y]), np.array([x0, y0]), atol=atol
            )
    ...
    def test_interpolation(self, rtol=1e-14, atol=1e-7):
        ...
        ci = ffun(x0, y0)
        np.testing.assert_allclose(
            ci, glpa.interpolate_gridded_scalar(x0, y0, f_at_g), atol=1e-1
        )
    ...
    def test_gridded_integration(self, atol=1e-10):
        ...
        for om in [0.1, 1.0, 10.0]:
            ...
            for n in range(nt):
                glpa.step_forward_with_gridded_uv(u_at_g, v_at_g, u_at_g, v_at_g, dt)

            # particles should be back to the same place
            np.testing.assert_allclose(
                np.array([glpa.x, glpa.y]), np.array([x0, y0]), atol=atol
            )
