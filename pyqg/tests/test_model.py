...
class PyqgModelTester(unittest.TestCase):
    def setUp(self):
        ...
        self.m = pyqg.QGModel(beta=0.0, U1=0.0, U2=0.0, filterfac=0.0)
    ...
    def test_advection(self, rtol=1e-14):
        ...
        for kwave in range(1, self.kwavemax):
            for lwave in range(1, self.lwavemax):
                ...
                tabs_mask.mask[1, -2 * lwave, 0] = 1
                # and make sure everything else is zero
                if np.any(np.isnan(tabs_mask.filled(0.0))):
                    print("Found NaNs")
                np.testing.assert_allclose(
                    tabs_mask.filled(0.0),
                    0.0,
                    rtol=0.0,
                    atol=rtol,
    ...
    def test_timestepping(self, rtol=1e-15):
        ...
        dqhdt = np.random.rand(*self.m.dqhdt.shape) + 1j * np.random.rand(
            *self.m.dqhdt.shape
        )
        self.m.dqhdt[:] = dqhdt
        ...
        np.testing.assert_allclose(
            self.m.qh, 1 * self.m.dt * dqhdt, err_msg="First timestep incorrect"
        )
        # step forward second time (should use AB2)
        self.m._forward_timestep()
        np.testing.assert_allclose(
            self.m.qh, 2 * self.m.dt * dqhdt, err_msg="Second timestep incorrect"
        )
        # step forward third time (should use AB3)
        self.m._forward_timestep()
        np.testing.assert_allclose(
            self.m.qh, 3 * self.m.dt * dqhdt, err_msg="Third timestep incorrect"
        )
