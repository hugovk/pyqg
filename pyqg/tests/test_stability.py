...
class LinearStabilityTester(unittest.TestCase):
    ...
    def test_two_layer_stability(self):
        ...
        # analytical results
        kb = sqrt(m.beta / (m.Us / 2.0))
        wv4 = m.wv2 ** 2
        kd2 = m.rd ** -2
        kd4 = kd2 ** 2
        kb4 = kb ** 4
        omg_ana = np.zeros_like(m.wv2) + 0.0j
        D = 1.0 + (4.0 * wv4 * (wv4 - kd4)) / (kb4 * kd4)

        fneg = D < 0.0
        omg_ana[fneg] = (
            1j
            * m.k[fneg]
            * (m.beta / (m.wv2[fneg] + kd2))
            * ((kd2 / (2.0 * m.wv2[fneg])) * sqrt(-D[fneg]))
        )
