...
def test_advect(rtol=1.0e-13):
    ...
    m = pyqg.QGModel(L=2.0 * np.pi, nx=256, U1=0)

    # there are some magic combinations that
    #   fails the test...try kx=12,ky=24
    #   should investigate what's going on...

    kx = np.array([1.0, 5.0, 10.0, 0.0, 24.0, 2.0])
    ky = np.array([2.0, 0.0, 10.0, 21.0, 12.0, 49.0])
    ...
    for i in range(kx.size):
        ...
        m.set_q1q2(np.cos(kx[i] * m.x + ky[i] * m.y), np.zeros_like(m.x))
