...
class GriddedLagrangianParticleArray2D(LagrangianParticleArray2D):
    ...
    def interpolate_gridded_scalar(self, x, y, c, order=1, pad=1, offset=0):
        ...
        return scipy.ndimage.map_coordinates(
            cp, [j, i], mode="constant", order=order, cval=np.nan
        )
    ...
    def step_forward_with_gridded_uv(self, U0, V0, U1, V1, dt, order=1):
        ...
        uv0fun = lambda x, y: (
            self.interpolate_gridded_scalar(x, y, U0p, pad=0, order=order, offset=pad),
            self.interpolate_gridded_scalar(x, y, V0p, pad=0, order=order, offset=pad),
        )
        uv1fun = lambda x, y: (
            self.interpolate_gridded_scalar(x, y, U1p, pad=0, order=order, offset=pad),
            self.interpolate_gridded_scalar(x, y, V1p, pad=0, order=order, offset=pad),
        )
