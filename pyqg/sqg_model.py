...
class SQGModel(model.Model):
    ...
    def __init__(
        self,
        beta=0.0,  # gradient of coriolis parameter
        Nb=1.0,  # Buoyancy frequency
        rd=0.0,  # deformation radius
        H=1.0,  # depth of layer
        U=0.0,  # max vel. of base-state
        **kwargs,
    ):
        """
        Parameters
    ...
    def _calc_eddy_time(self):
        ...
        ens = 0.5 * self.H * spec_var(self, self.wv2 * self.ph)
